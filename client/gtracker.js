...
const UPDATE_TOKEN = "";
const WIPE_TOKEN = "";
const GOLD_WEIGHTINGS = Object.fromEntries(Object.entries({
    "gold_block": 9,
    "raw_gold_block": 9,
    "gold_ore": 1,
...
const log = (msg, colour = 0xf, forVerboseMode = false) => {
    if (forVerboseMode && !FileSys.getConfigValue("verbose"))
        return;
...
const getAuthHeader = (token) => new Map([["Authorization", `Bearer ${token}`]]);
...
const getGoldCount = (inv = null, container = false, slots = null) => {
...
const dropSlotListener = JsMacros.on("DropSlot", JavaWrapper.methodToJavaAsync((ctx) => {
    const inv = Player.openInventory();
    if (ctx.slot === -999)
        return;
...
const openContainerListener = JsMacros.on("OpenContainer", JavaWrapper.methodToJavaAsync((ctx) => {
    const containerTitle = ctx.inventory.getContainerTitle();
    if (!containerTitle || containerTitle === "Crafting")
        return;
...
const closeContainerListener = JsMacros.on("OpenScreen", JavaWrapper.methodToJavaAsync((ctx) => {
    if (!ctx.screenName && ecm.wasOpen) {
        ...
        if (currentMainGoldCount < ecm.mainGoldCount) {
            ...
            FileSys.setConfigValue("echestGoldCount", ecm.contGoldCount - Math.abs(ecm.mainGoldCount - currentMainGoldCount));
        }
        else {
            redistributed = false;
        ...
        if (getGoldCache() === totalGoldCount) {
            DEBUG &&
                log(`No need to push new count${redistributed ? ", but the echest/inventory was redistributed." : "."}`, 0xf, true);
        }
        else {
            setGoldCache(totalGoldCount);
            updateGoldCount();
...
const attackBlockListener = JsMacros.on("AttackBlock", JavaWrapper.methodToJavaAsync((evt) => {
    currentAttackedBlock = evt.block;
}));
const breakStorageListener = JsMacros.on("SendPacket", JsMacros.createEventFilterer("SendPacket").setType("PlayerActionC2SPacket"), JavaWrapper.methodToJavaAsync((evt) => {
